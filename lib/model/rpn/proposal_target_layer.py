...
class _ProposalTargetLayer(nn.Module):
    ...
    def forward(self, all_rois_left, gt_boxes_left, \
    ...
    def _sample_rois_pytorch(self, all_rois_left, gt_boxes_left, \
        ...
        gt_dim_orien_batch = all_rois_left.new(batch_size, rois_per_image, 5).zero_()
        gt_kpts_batch = all_rois_left.new(batch_size, rois_per_image, 6).zero_()
        # Guard against the case when an image has fewer than max_fg_rois_per_image
        # foreground RoIs
        for i in range(batch_size):

            fg_inds = torch.nonzero((max_overlaps_left[i] >= cfg.TRAIN.FG_THRESH)).view(-1)
        ...
        return labels_batch, rois_batch_left, gt_assign_batch_left, \
               bbox_targets_left, dim_orien_target, kpts_targets, kpts_weight, bbox_inside_weights_left
