...
def vis_detections(im, class_name, dets, thresh=0.8, kpts=None):
    ...
    for i in range(np.minimum(100, dets.shape[0])):
        ...
        score = dets[i, -1]
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 0, 200), 1)
            cv2.putText(im, '%s: %.3f' % (class_name, score), (bbox[0], bbox[1]-15), cv2.FONT_HERSHEY_PLAIN,
                        1.0, (0, 0, 255), thickness=1)
            if kpts is not None:
                kpt = int(np.round(kpts[i, 0]))
                kpts_type = int(kpts[i, 1])
                if kpts_type == 0:
                    cv2.circle(im, (kpt, bbox[3]), 3, (0,0,255), -1) # R
                if kpts_type == 1:
                    cv2.circle(im, (kpt, bbox[3]), 3, (0,255,0), -1) # G
                if kpts_type == 2:
                    cv2.circle(im, (kpt, bbox[3]), 3, (255,0,0), -1) # B
                if kpts_type == 3:
                    cv2.circle(im, (kpt, bbox[3]), 3, (0,255,255), -1) # Y

                cv2.circle(im, (int(kpts[i,3]), bbox[3]), 3, (255,0,255), -1) # left
                cv2.circle(im, (int(kpts[i,4]), bbox[3]), 3, (255,0,255), -1) # right
