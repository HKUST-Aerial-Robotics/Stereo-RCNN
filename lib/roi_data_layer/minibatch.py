...
def get_minibatch(roidb, num_classes, TRAIN=False):
  ...
  im_blob_left, im_blob_right, im_scales = _get_image_blob(roidb, random_scale_inds, TRAIN)
  ...
  gt_dis = np.empty((len(gt_inds), 2), dtype=np.float32)
  gt_dis[:,0] = roidb[0]['poses'][gt_inds][:,3]*im_scales[0]
  gt_dis[:,1] = (roidb[0]['poses'][gt_inds][:,3]-roidb[0]['poses'][gt_inds][:,4])*im_scales[0]

  gt_dim_orien = np.empty((len(gt_inds), 5), dtype=np.float32)
  gt_dim_orien[:, 0:3] = roidb[0]['poses'][gt_inds][:,5:8]
  for i in range(gt_dim_orien.shape[0]):
    gt_dim_orien[i, 3] = math.sin(roidb[0]['poses'][gt_inds][i,8])
    gt_dim_orien[i, 4] = math.cos(roidb[0]['poses'][gt_inds][i,8])
...
def _get_image_blob(roidb, scale_inds, TRAIN=False):
