...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
      ...
      args.imdbtrainval_name = "kitti_trainval"
  ...
  imdb, roidb, ratio_list, ratio_index = combined_roidb(args.imdbtrainval_name)
  ...
  # make variable 
  im_left_data = Variable(im_left_data)
  im_info = Variable(im_info)
  num_boxes = Variable(num_boxes)
  gt_boxes_left = Variable(gt_boxes_left)
  gt_boxes_right = Variable(gt_boxes_right)
  gt_boxes_merge = Variable(gt_boxes_merge)
  gt_dis = Variable(gt_dis)
  gt_dim_orien = Variable(gt_dim_orien)
  gt_kpts = Variable(gt_kpts) 
  ...
  if cfg.TRAIN.MULTI_TASK_LOSS:
    uncert_count = 6
    uncert = torch.rand(uncert_count)
    uncert = Variable(uncert.cuda(), requires_grad=True)
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      gt_boxes_left.data.resize_(data[2].size()).copy_(data[2])
      gt_boxes_right.data.resize_(data[3].size()).copy_(data[3])
      gt_boxes_merge.data.resize_(data[4].size()).copy_(data[4])
      gt_dis.data.resize_(data[5].size()).copy_(data[5])
      ...
      rois_left,ls_prob, bbox_pred, dim_orien_pred, kpts_prob, \
      ...
      if cfg.TRAIN.MULTI_TASK_LOSS:
        ...
        print('uncert', uncert_data[0], uncert_data[1], uncert_data[2], uncert_data[3], uncert_data[4], uncert_data[5]) 
      else:
        loss = 10.0*rpn_loss_cls.mean() + 10.0* rpn_loss_box_left_right.mean()+ \
               RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() + \
               RCNN_loss_dim_orien.mean() + RCNN_loss_kpts
      ...
      if step % args.disp_interval == 0:
        ...
        if args.mGPUs:
          ...
          loss_rpn_box_left_right = rpn_loss_box_left_right.mean().data[0]
          loss_rcnn_cls = RCNN_loss_cls.mean().data[0]
          loss_rcnn_box = RCNN_loss_bbox.mean().data[0]
          loss_rcnn_dim_orien = RCNN_loss_dim_orien.mean().data[0]
          loss_rcnn_kpts = RCNN_loss_kpts
          fg_cnt = torch.sum(rois_label.data.ne(0))
          bg_cnt = rois_label.data.numel() - fg_cnt
        else: 
          loss_rpn_cls = rpn_loss_cls.data[0].scale
        ...
        print("\t\t\trpn_cls: %.4f, rpn_box_left_right: %.4f, rcnn_cls: %.4f, rcnn_box_left_right %.4f,dim_orien %.4f, kpts %.4f" \
